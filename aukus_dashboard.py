...
if page == "Why This Matters":
    ...
    for col, card in zip([nc1, nc2, nc3, nc4, nc5], cards):
        with col:
            st.markdown(f"""
            <div class='news-card'>
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    for col_p, flow_type in [(ci1, "Import"), (ci2, "Export")]:
        with col_p:
            ...
            fig_f = px.line(ann, x="year", y=ann["trade_value"] / 1e9,
                            ...
                            markers=True, title=f"Australia {flow_type}s by Partner",
                            labels={"y": "USD Billions", "x": "Year"})
            fig_f = add_event_lines(fig_f)
            fig_f.update_layout(**PLOTLY_LAYOUT, height=360)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    with col_hm:
        ...
        fig_hm = go.Figure(data=go.Heatmap(
            ...
            colorbar=dict(title="% Change", tickfont=dict(color="#c8d6f0"), titlefont=dict(color="#c8d6f0")),
        ))
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    fig_comp = px.bar(
        comp_avg, x="commodity", y=comp_avg["trade_value"] / 1e9,
        color="regime",
        barmode="group",
        color_discrete_map={"Pre-AUKUS Regime": "#2d4a7a", "Post-AUKUS Regime": "#3a7bd5"},
